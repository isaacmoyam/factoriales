	...
	<body>
		...
		<?php
			if(isset($_GET['uno'])&& isset($_GET['dos'])){ //comprueba si los nombres uno y dos estan en el array _GET
				...
				if($uno<=$dos){  //si la variable uno es menor o igual a la variable dos...
					for($i=$uno;$i<=$dos;$i++){ //de manera creciente se iran mostrando tanto numeros como factoriales
						echo "<tr>";
						echo "<td>$i</td>";
						echo "<td>".factorial($i)."</td>"; //se concatena el retorno de la funcion factorial y se le pasa como parametro la i del for de arriba
						echo "</tr>";
					}
				}else{
					if($uno>$dos){ //si la variable uno es mayor a la variable dos...
						for($i=$uno;$i>=$dos;$i--){ //de manera decreciente se iran mostrando tanto numeros como factoriales
							echo "<tr>";
							echo "<td>$i</td>";
